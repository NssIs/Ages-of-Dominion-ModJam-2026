...
/** Draws the selected mine's live occupants and timed yield above its world position. */
public final class RtsMineStatusRenderer {
    ...
    private RtsMineStatusRenderer() {
    }

    public static void register() {
        NeoForge.EVENT_BUS.addListener(RtsMineStatusRenderer::onRenderGui);
    }
    ...
    private static void onRenderGui(RenderGuiEvent.Post event) {
        ...
        String title = "stone".equalsIgnoreCase(status.resource())
                ? "Quarry" : resourceLabel(status.resource()) + " Mine";
        String occupants = "Inside " + status.workersInside() + " / " + status.capacity();
        String yield = "Yield " + status.output() + " " + status.resource()
                + (status.bonusOutput() > 0 && status.bonusResource() != null
